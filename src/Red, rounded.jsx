...
export default function App() {
  ...
  // We pull gallery images from CMS if they exist, otherwise use high-converting placeholders
  ...
  return (
    <div className="font-sans text-[#2A2626] bg-[#F9F6F0] min-h-screen selection:bg-[#C82A2A] selection:text-white antialiased overflow-x-hidden">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 border-b ${isScrolled ? 'bg-[#2A2626]/95 backdrop-blur-md border-white/10 py-3 shadow-2xl' : 'bg-transparent border-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer hover:scale-105 transition-transform" onClick={() => window.scrollTo(0,0)}>
            ...
            <div className="text-xl md:text-2xl font-black tracking-tighter uppercase text-white drop-shadow-md hidden sm:block">
              La Pizza <span className="text-[#C82A2A]">di Sabi</span>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-8 font-bold text-xs tracking-widest uppercase text-white drop-shadow-md">
            <button onClick={() => scrollToSection('gallery')} className="hover:text-[#C82A2A] transition-colors">Beleving</button>
            <button onClick={() => scrollToSection('pricing')} className="hover:text-[#C82A2A] transition-colors">Pakketten</button>
            <button onClick={() => scrollToSection('menu')} className="hover:text-[#C82A2A] transition-colors">Menu</button>
            <button onClick={() => scrollToSection('faq')} className="hover:text-[#C82A2A] transition-colors">FAQ</button>
            <button onClick={() => scrollToSection('contact')} className="px-6 py-3 bg-[#C82A2A] text-white hover:bg-white hover:text-[#C82A2A] transition-colors shadow-lg shadow-red-900/20">Beschikbaarheid Checken</button>
          </div>
        ...
        {mobileMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-[#2A2626] text-white p-8 flex flex-col space-y-6 lg:hidden border-b-4 border-[#C82A2A] shadow-2xl">
            ...
            <button onClick={() => scrollToSection('contact')} className="text-xl font-bold uppercase text-left text-[#C82A2A] pt-4 border-t border-white/10">Beschikbaarheid Checken</button>
          </div>
        )}
      </nav>

      {/* Hero Sectie (The Hook) */}
      <section className="relative h-screen flex flex-col justify-center items-center bg-[#2A2626] pt-10 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mt-12 flex flex-col items-center">
          ...
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 px-4 py-1.5 rounded-full mb-8">
            ...
            <span className="text-white text-xs font-bold tracking-widest uppercase">Beoordeeld met 5.0 door 50+ gasten</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center w-full sm:w-auto">
            <button onClick={() => scrollToSection('contact')} className="px-8 py-5 bg-[#C82A2A] text-white font-black uppercase tracking-widest flex items-center justify-center gap-3 text-sm md:text-base hover:bg-white hover:text-[#C82A2A] transition-all duration-300 shadow-[0_10px_30px_rgba(200,42,42,0.4)]">
              Offerte Aanvragen <ArrowRight size={20} />
            </button>
            <button onClick={() => scrollToSection('pricing')} className="px-8 py-5 bg-transparent border-2 border-white text-white font-black uppercase tracking-widest flex items-center justify-center gap-3 text-sm md:text-base hover:bg-white/10 transition-all duration-300">
              Bekijk Pakketten
            </button>
          </div>
        </div>
      ...
      {/* The Vibe (Gallery moved up to sell the experience instantly) */}
      <section id="gallery" className="py-24 px-6 bg-[#F9F6F0]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {gallery.map((photo, i) => (
              <div key={i} className={`bg-white p-2 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 hover:z-10 relative cursor-crosshair
      ...
      {/* The Investment (Pricing moved up because event planners want to know the cost quickly) */}
      <section id="pricing" className="py-24 px-6 bg-white border-y border-stone-200">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8 items-stretch">
            {packages.map((pkg, i) => (
              <div key={i} className={`p-8 md:p-10 flex flex-col relative transition-all duration-300 border-2
                ${pkg.featured 
                  ? 'bg-[#2A2626] text-white border-[#2A2626] lg:-translate-y-4 shadow-[0_20px_40px_rgba(0,0,0,0.2)] z-10' 
                  : 'bg-[#F9F6F0] text-[#2A2626] border-stone-200 hover:border-stone-300 shadow-sm hover:shadow-lg'}`}>
                
                {pkg.featured && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#C82A2A] text-white font-black uppercase text-[10px] tracking-widest py-1.5 px-6 whitespace-nowrap">Meest Gekozen</div>}
                
                <h3 className="text-3xl font-black uppercase mb-2 tracking-tight">{pkg.title}</h3>
                <p className={`text-sm mb-6 italic min-h-[40px] ${pkg.featured ? 'text-stone-400' : 'text-stone-500'}`}>{pkg.description}</p>
                ...
                <ul className="space-y-4 mb-10 flex-1">
                  {pkg.features?.map((f, j) => (
                    <li key={j} className="flex items-start gap-3 text-base font-medium">
                      <CheckCircle size={20} className={`shrink-0 ${pkg.featured ? 'text-[#C82A2A]' : 'text-[#C82A2A]'}`} /> 
                      <span className={pkg.featured ? 'text-stone-200' : 'text-stone-700'}>{f}</span>
                    </li>
                  ))}
                </ul>
                <button onClick={() => scrollToSection('contact')} className={`w-full py-4 font-black uppercase text-sm tracking-widest transition-all ${pkg.featured ? 'bg-[#C82A2A] text-white hover:bg-white hover:text-[#C82A2A]' : 'bg-[#2A2626] text-white hover:bg-[#C82A2A]'}`}>Boek {pkg.title}</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12 mb-20">
            {[
              ...
              <div key={i} className="flex flex-col sm:flex-row items-center sm:items-start gap-6 p-4 border-b border-stone-800 pb-8 hover:bg-stone-800/50 transition-colors duration-300">
                <div className="w-full sm:w-32 sm:h-32 aspect-square bg-white p-1.5 shadow-lg rotate-1 shrink-0">
          ...
          {/* Videos naturally flow here - Seeing the menu being cooked */}
          <div className="text-center mb-8">
            <h3 className="text-2xl font-black uppercase tracking-widest text-[#C82A2A]">450 Graden. 90 Seconden.</h3>
          </div>
          ...
          <div className="flex gap-6 overflow-x-auto px-[5vw] md:px-0 md:justify-center pb-8 pt-4 snap-x snap-mandatory scroll-smooth hide-scrollbar w-full">
            {reels.map((item, i) => (
              <div key={i} className="relative flex-none w-[75vw] md:w-[280px] aspect-[9/16] bg-[#2A2626] p-2 shadow-2xl snap-center shrink-0 transition-transform duration-300 hover:scale-105 border border-stone-700">
      ...
      {/* Trust (Testimonials) */}
      <section className="py-24 bg-[#C82A2A] text-white px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12">
            {testimonials.map((review, i) => (
              <div key={i} className="flex flex-col items-center text-center p-10 bg-[#A61E1E] shadow-xl relative">
                {/* Big decorative quote mark */}
                <div className="text-8xl text-red-900/30 font-serif absolute top-4 left-6 leading-none">"</div>
                
                <div className="bg-white p-1 -rotate-3 mb-6 shadow-md z-10 relative mt-2">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col gap-4">
            {[
              ...
              <details key={i} className="group bg-white p-6 shadow-sm border border-stone-200 hover:border-[#C82A2A] cursor-pointer transition-all duration-300">
                <summary className="text-lg md:text-xl font-black tracking-wide list-none flex justify-between items-center outline-none [&::-webkit-details-marker]:hidden text-[#2A2626]">
                  {faq.q}
                  <span className="transition-transform duration-300 group-open:rotate-180 shrink-0 ml-4 text-[#C82A2A]">
      ...
      <section id="contact" className="py-24 px-4 md:px-6 bg-white border-t border-stone-200">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-16">
            <h2 className="text-5xl md:text-7xl font-black uppercase mb-4 tracking-tighter leading-[1] text-[#2A2626]">Check Beschikbaarheid</h2>
            ...
            <div className="flex items-center justify-center gap-2 text-stone-600 font-medium">
              <Phone size={18} className="text-[#C82A2A]" />
              <span>Snel antwoord? Bel ons op:</span>
              <a href="tel:+31612345678" className="font-black text-[#2A2626] hover:text-[#C82A2A] transition-colors ml-1">
                +31 6 12 34 56 78
              </a>
            </div>
          </div>
          ...
          <form 
            ...
            method="POST" 
            className="bg-[#F9F6F0] p-6 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-t-8 border-[#C82A2A] flex flex-col gap-8 w-full box-border relative"
          >
            ...
            <div className="grid md:grid-cols-2 gap-8 relative z-10">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-black text-stone-500 uppercase tracking-widest">Jouw Naam</label>
                <input required type="text" name="Naam" className="w-full p-3 border-b-2 border-stone-300 focus:outline-none focus:border-[#C82A2A] transition-colors bg-transparent text-xl font-black text-[#2A2626]" placeholder="Vul hier in..." />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs font-black text-stone-500 uppercase tracking-widest">E-mail of Telefoon</label>
                <input required type="text" name="Contactgegevens" className="w-full p-3 border-b-2 border-stone-300 focus:outline-none focus:border-[#C82A2A] transition-colors bg-transparent text-xl font-black text-[#2A2626]" placeholder="Vul hier in..." />
              </div>
            </div>

            <div className="grid md:grid-cols-2 gap-8 relative z-10">
              <div className="flex flex-col gap-2 min-w-0">
                <label className="text-xs font-black text-stone-500 uppercase tracking-widest flex items-center gap-2"><Calendar size={14}/> Datum Evenement</label>
                <input required type="date" name="Datum" className="w-full p-3 border-b-2 border-stone-300 focus:outline-none focus:border-[#C82A2A] transition-colors bg-transparent text-xl font-black text-[#2A2626] block cursor-pointer" />
              </div>
              
              <div className="flex flex-col gap-2 min-w-0">
                <label className="text-xs font-black text-stone-500 uppercase tracking-widest flex items-center gap-2"><Users size={14}/> Aantal Personen</label>
                <select required name="Aantal Personen" className="w-full p-3 border-b-2 border-stone-300 focus:outline-none focus:border-[#C82A2A] transition-colors bg-transparent text-xl font-black text-[#2A2626] cursor-pointer" defaultValue="">
            ...
            <div className="flex flex-col gap-2 relative z-10">
              <label className="text-xs font-black text-stone-500 uppercase tracking-widest">Locatie & Extra wensen</label>
              <textarea required name="Bericht" rows="3" className="w-full p-3 border-b-2 border-stone-300 focus:outline-none focus:border-[#C82A2A] transition-colors bg-transparent resize-none text-xl font-medium text-[#2A2626]" placeholder="Waar is het feest en zijn er bijzonderheden?"></textarea>
            </div>

            <button type="submit" className="w-full py-6 mt-4 bg-[#C82A2A] text-white font-black text-lg uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-[#2A2626] transition-colors relative z-10 shadow-xl">
              Verstuur Offerte Aanvraag <ArrowRight size={24} />
            </button>
          </form>
        </div>
      </section>

      {/* Final Footer - Full Color Logo! */}
      <footer className="bg-[#2A2626] text-stone-400 py-16 px-6 border-t-4 border-[#C82A2A]">
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center text-center">
          <div className="flex items-center gap-3 mb-8 cursor-pointer hover:scale-105 transition-transform" onClick={() => window.scrollTo(0,0)}>
            {/* Logo is now full color! No grayscale opacity. */}
      ...
      <a 
        ...
        className="md:hidden fixed bottom-6 right-6 bg-[#C82A2A] text-white p-4 rounded-full shadow-[0_10px_20px_rgba(200,42,42,0.4)] z-50 flex items-center justify-center active:scale-95 transition-transform"
